...
One liner install : 
```bash
sudo apt install unzip wget -y && wget https://github.com/lostallmymoney/Razer_Mouse_Linux/archive/refs/heads/master.zip -O MoveMouseLinux.zip && unzip -o MoveMouseLinux.zip && cd MoveMouseLinux-master && sh install.sh && cd .. && rm -rf MoveMouseLinux-master MoveMouseLinux.zip
```
